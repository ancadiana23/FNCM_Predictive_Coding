...
class Model:
    ...
    def apply_images(self, images, training):
        ...
        errors = np.zeros([768], dtype=np.float32)
        ...
        #patch = self.reconstruct(rs, level = 1)
        #plt.imshow(patch)

        return rs, r_tds, rh, error_tds, errors, Ur
    ...
    def train(self, dataset):
        ...
        for i in range(patch_size):
            ...
            #        plt.subplot(1, 3, img_index + 1)
            #        plt.imshow(images[img_index].reshape((16, 16)))
            #    plt.show()
            rs, r_tds, rh, error_tds, errors, Ur = self.apply_images(images, training=True)
